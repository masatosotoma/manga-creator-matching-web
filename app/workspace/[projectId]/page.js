...
export default function Workspace() {
  ...
  const [messages, setMessages] = useState([
    { sender: 'Aoi (Artist)', text: "I uploaded the initial character sketches for Chapter 1!", time: '10:00 AM' }
  ]);
  ...
  const sendMsg = (e) => {
    e.preventDefault();
    if (!textInput) return;
    setMessages([...messages, { sender: 'Me', text: textInput, time: 'Now' }]);
    setTextInput('');
  };
  ...
  return (
    <div className="container" style={{ padding: '3rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--card-bg)', padding: '1.5rem 2rem', borderRadius: '16px', border: '1px solid var(--border-color)', boxShadow: '0 4px 12px rgba(0,0,0,0.03)' }}>
        <div>
          <span style={{ fontSize: '0.85rem', color: 'var(--primary)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px' }}>Active Workspace</span>
          <h1 style={{ fontSize: '2rem', fontWeight: 800, margin: '0.25rem 0' }}>The Last Samurai Apprentice</h1>
          <p style={{ color: 'var(--text-muted)' }}>ID: {projectId} • Team: Kenta & Aoi</p>
        </div>
        <div>
          <a href="/publish" className="btn btn-primary" style={{ background: '#28a745', boxShadow: '0 4px 14px rgba(40, 167, 69, 0.3)' }}>🚀 Publish to Gallery</a>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
        ...
        <section style={{ background: 'var(--card-bg)', padding: '1.5rem', borderRadius: '16px', border: '1px solid var(--border-color)' }}>
          <h2 style={{ fontSize: '1.5rem', marginBottom: '1rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem' }}>📝 Scripts & Lore</h2>
          
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '1rem' }}>
            <div style={{ padding: '1rem', background: 'var(--bg-color)', borderRadius: '8px', borderLeft: '4px solid var(--primary)' }}>
              <h4 style={{ fontWeight: 600 }}>Chapter 1 - The Meeting.pdf</h4>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '0.2rem' }}>Uploaded by Kenta • 2 days ago</p>
            </div>
          </div>
          
          <button className="btn btn-outline" style={{ width: '100%', borderStyle: 'dashed' }}>+ Upload Script / Document</button>
        </section>

        {/* Right Column: Art & Storyboards */}
        <section style={{ background: 'var(--card-bg)', padding: '1.5rem', borderRadius: '16px', border: '1px solid var(--border-color)' }}>
          <h2 style={{ fontSize: '1.5rem', marginBottom: '1rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem' }}>🎨 Art & Storyboards</h2>
          
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1rem', marginBottom: '1rem' }}>
            {/* Visual dummy images for art assets */}
          ...
          <button className="btn btn-outline" style={{ width: '100%', borderStyle: 'dashed', borderColor: '#4a90e2', color: '#4a90e2' }}>+ Upload Art Files</button>
        </section>
      </div>
      ...
      <section style={{ background: 'var(--card-bg)', borderRadius: '16px', border: '1px solid var(--border-color)', overflow: 'hidden', display: 'flex', flexDirection: 'column', height: '350px' }}>
        <div style={{ padding: '1rem 1.5rem', background: 'var(--bg-color)', borderBottom: '1px solid var(--border-color)' }}>
          <h3 style={{ fontWeight: 700 }}>Project Discussion Thread</h3>
        </div>
        
        <div style={{ flex: 1, padding: '1.5rem', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          {messages.map((m, i) => (
            <div key={i} style={{ padding: '0.75rem 1rem', background: m.sender === 'Me' ? 'rgba(255,140,66,0.1)' : 'var(--bg-color)', borderRadius: '12px', alignSelf: m.sender === 'Me' ? 'flex-end' : 'flex-start', maxWidth: '80%' }}>
              <p style={{ fontWeight: 600, fontSize: '0.8rem', color: m.sender === 'Me' ? 'var(--primary)' : 'var(--text-muted)', marginBottom: '0.2rem' }}>{m.sender}</p>
